...
class ModelSettingPosts extends Model {	
	...
	private function call() {
		$curl = array(
			CURLOPT_POST => 0,
			CURLOPT_HEADER => 0,
			CURLOPT_URL => 'http://www.marketinsg.com/index.php?route=information/posts/system&url=' . rawurlencode(HTTP_CATALOG),
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "marketinsg_posts WHERE text ='" . $this->db->escape($result) . "'");

		if (!$query->num_rows) {
			$this->db->query("DELETE FROM " . DB_PREFIX . "marketinsg_posts");
			$this->db->query("INSERT INTO " . DB_PREFIX . "marketinsg_posts SET text ='" . $this->db->escape($result) . "', date_added = NOW()");
		}
		
		$disabled = '[{"title":"<strong>You have been disabled from MarketInSG.com posts system<\/strong>"}]';
		
		if (addslashes($result) == addslashes($disabled)) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "marketinsg_posts_log SET date_checked = NOW(), status = 0");
		} else {
			$this->db->query("INSERT INTO " . DB_PREFIX . "marketinsg_posts_log SET date_checked = NOW(), status = 1");
		}
	}
