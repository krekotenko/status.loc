...
class ControllerInformationPosts extends Controller {
	public function index() {
		...
		foreach ($all_posts as $posts) {
			$data['all_posts'][] = array (
				'title' 		=> $posts['title'],
				'image'			=> $this->model_tool_image->resize($posts['image'], 100, 100),
	...
	public function posts() {
		...
		if ($posts) {
			...
			$this->document->setDescription($posts['meta_description']);
			$this->document->setKeywords($posts['meta_keywords']);
