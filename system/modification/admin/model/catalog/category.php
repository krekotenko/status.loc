...
*/
		...
		$b = "SELECT url_alias_id FROM " . DB_PREFIX . "url_alias WHERE query = 'category_id=" . (int)$category_id . "'";
		$s = $this->db->query($b);

		$id = $s->row["url_alias_id"];

		$this->db->query("UPDATE " . DB_PREFIX . "url_alias SET `keyword` = '" . $this->db->escape($data['keyword']) . "' WHERE `url_alias_id` = $id");
